...
class Taxonomy(unittest.TestCase):
    def test_find_taxon_ids_reads_nested_taxon_information(self):
        c = make_client([FakeResp(200, {"data": [
            {"id": 46673, "taxonInformation": {"taxonId": 100034}},
            {"id": 46674, "taxonInformation": {"taxonId": 100034}},
            {"id": 99, "taxonInformation": {"taxonId": 5}},
        ]})])
        self.assertEqual(c.find_taxon_ids("blå kärrhök"), [100034, 5])
    ...
    def test_get_taxon_posts_id_list_to_taxa(self):
        c = make_client([FakeResp(200, [{"taxonId": 1}])])
        c.get_taxon(1)
        call = c._session.calls[0]
        self.assertEqual(call["method"], "POST")
        self.assertEqual(call["url"],
                         "https://api.artdatabanken.se/taxonservice/v1/taxa")
        self.assertEqual(call["json"], {"taxonIds": [1]})
        self.assertEqual(call["headers"]["Ocp-Apim-Subscription-Key"], "TAXKEY")

    def test_taxon_parent_ids_strips_root(self):
        c = make_client([FakeResp(200, {"taxonIds": [0, 5000001, 100034]})])
        self.assertEqual(c.taxon_parent_ids(100034), [5000001, 100034])
