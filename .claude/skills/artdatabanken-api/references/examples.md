...
# Inspect a hit if a name is ambiguous — note the taxon id is nested and each
# row is one *name*, so de-dupe on taxonInformation.taxonId:
client.find_taxa("Pieris")
# → [{"id": 41200, "name": "Pieris", "category": {"name": "Släkte"},
#     "taxonInformation": {"taxonId": 101657, "recommendedScientificName": "Pieris"}},
#    {"id": 41230, "name": "rovfjäril", "taxonInformation": {"taxonId": 101660,
#     "recommendedScientificName": "Pieris rapae"}}, ...]

# Full detail for known ids (POST /taxa — there is no GET /taxa/{id}):
client.get_taxa([101657, 101660])
client.get_taxon(101660)          # single-id convenience
```
